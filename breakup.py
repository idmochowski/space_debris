...
class _rv_dist(stats.rv_continuous):
    def _cdf(self, x):
        return np.where(x < LB, 0., 1 - np.sign(x)*np.abs(x) ** -1.71 / (np.sign(LB)*(np.abs(LB))** -1.71))
...
def breakup(ep, p1, p2):
    ...
    if catastrophRatio<40:
        M = p2.mass*dv
        Lc = np.linspace(LB, 1.)
        num = int(0.1 * M ** 0.75 * (np.sign(LB)*(np.abs(LB))** -1.71))
    else:
    ...
    # dv velocity vectors
    try:
        dv_vec = np.array(list(zip(p[:,0] * dv, p[:,1] * dv, p[:,2] * dv)))
    except:
        # if len(p.shape) != 2:
        return
        #     p = p[None,:]
...
#    plt.show()
#
    fragments = np.array(list(zip(d, A, Am, m, dv, dv_vec[:,0], dv_vec[:,1], dv_vec[:,2])))
