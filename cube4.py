...
numOfExperiments = int(re.sub("[^0-9]", "",next(fParam)))

# length of experiment in years, standard setting 200 years
timeHorizon = int(re.sub("[^0-9]", "",next(fParam)))

# Russia
removedObjectsPerYear1 = int(re.sub("[^0-9]", "",next(fParam)))

# USA
removedObjectsPerYear2 = int(re.sub("[^0-9]", "",next(fParam)))

# China
removedObjectsPerYear3 = int(re.sub("[^0-9]", "",next(fParam)))

# EU
removedObjectsPerYear4 = int(re.sub("[^0-9]", "",next(fParam)))

#how often we remove objects - 1 -> every year, 10 -> every 10 years
removeHorizon = int(re.sub("[^0-9]", "",next(fParam)))
...
#  filtering only objects in LEO
    ...
    listOagentThreatsfRemovedObjectsFinal = []
            ...
            for p1, p2 in combinations(volume, 2):
                ...
                if tuple(p1._v) == tuple(p2._v):
                    ...
                    if P > 0 and virtualRun == 1:
                        ...
                        if p1.name.strip() in importantAssets and p2.name.strip() in importantAssets:
                        ...
                        elif p1.name.strip() in importantAssets and not p2.name.strip() in importantAssets:
                            plnt1 = importantAssets[p1.name.strip()]
                            collisionRiskCouplesVirtual.append([p1.name.strip(), p2.name.strip(), plnt1[5].strip(), "ROW", P])
                        elif not p1.name.strip()in importantAssets and p2.name.strip() in importantAssets:
                            plnt2 = importantAssets[p2.name.strip()]
                        ...
                        for agentIndex in range(0,len(agentList)):
                            if p1.name.strip() in importantAssets:
                                plnt1 = importantAssets[p1.name.strip()]
                                if plnt1[5].strip() in agentListWithEU[agentIndex]:
                                    agentThreats=list(agentThreats)
                        ...
                        for agentIndex in range(0,len(agentList)):
                            if p2.name.strip() in importantAssets:
                                plnt2 = importantAssets[p2.name.strip()]
